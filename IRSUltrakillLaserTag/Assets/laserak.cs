...
public class laserak : MonoBehaviour
{
    ...
    void Update()
    {
        if (GameObject.Find("Player").GetComponent<PlayerController>().selectedGun == "ak") {
        GetComponent<Renderer>().enabled = true;
        lr.SetPosition(0, new Vector3(0, 0, 0.066f));
        lr.SetPosition(1, new Vector3(0, -10, 0));
        if (Input.GetMouseButton(0) && Time.frameCount % 5 == 0) {
            StartCoroutine(shootLaserAK());
            //im so sorry
